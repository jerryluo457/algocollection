...
#  edgeList format: [[inLabel, outLabel, capacity], ...]
    ...
    def bfs(self, source, sink):
        ...
        while queue:
            u = queue.popleft()
            for v, capacity in self.graph[u].items():  # use residual graph
...
def run_NF():
    ...
    for _ in range(number_inputs):
        ...
        for _ in range(edges):
            u, v, cap = input().split()
            edgeList.append([int(u), int(v), int(cap)]) 

        network = Network(edgeList) #start network
...
def build_graph():
    """
    inputs a list of edges, outputs a graph object
    """
    pass
